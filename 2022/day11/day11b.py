...
class Monkey:
    ...
    def operation(self):
        for index, item in enumerate(self.items):
            ...
            self.items[index] = self.operator(item, self.operation_value) % modulo
...
# Couldn't be bothered to parse the file today
...
# HINT: The internet helped me to discover that this was a modulo problem
# calculate modulo of test_divisors
modulo = 1
...
# Multiply total items inspected by the two cheekiest monkeys
monkeys.sort(key=operator.attrgetter('total_items_inspected'), reverse=True)
print(monkeys[0].total_items_inspected * monkeys[1].total_items_inspected) # 24389045529